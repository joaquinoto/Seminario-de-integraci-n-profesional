...
export default function CategoriesPage() {
  ...
  const closeModal = () => { setModalOpen(false); setEditing(null); dispatch(clearCategoryActionState()); };
  ...
  return (
    <div className="cats-page">
      ...
      <div className="cats-content">
        ...
        {status === 'succeeded' && filtered.length === 0 && (
          <EmptyState
            ...
            description={search ? 'Probá con otra búsqueda.' : 'Creá la primera categoría para empezar.'}
            action={
              isOwner(user) && !search && (
            ...
            {filtered.map((cat, i) => (
              ...
              >
                <div className="cat-row-main">
                  <div className="cat-color-chip" style={{ background: colorForId(cat.id) }} />
                ...
                <div className="cat-row-meta">
                  ...
                  {isOwner(user) && (
                    <div className="cat-actions">
                      <ActionBtn variant="edit"   onClick={() => openEdit(cat)}     title="Editar" />
                      <ActionBtn variant="delete" onClick={() => setConfirmId(cat.id)} title="Desactivar" disabled={!cat.active} />
                    </div>
                  )}
                </div>
              </div>
      ...
      <ConfirmDialog
        ...
        message={`¿Desactivar "${confirmTarget?.name}"? Los productos asociados no se verán afectados, pero la categoría no aparecerá en nuevos formularios.`}
      ...
      <style>{`
        ...
        .cat-row-main { display: flex; align-items: center; gap: 14px; flex: 1; min-width: 0; }
